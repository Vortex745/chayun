<template>
  <div class="page-container">
    ...
    <div class="table-container">
      ...
      >
            ...
            <div v-if="['已发货', '已完成'].includes(row.state)" class="delivery-wrapper">
              ...
              <div v-else class="express-info">
                ...
                <div class="express-number">
                  <el-icon style="margin-right: 2px; vertical-align: -1px;">
                    <Van/>
                  </el-icon>
                  {{ row.trackingNumber }}
                </div>
        ...
        <el-table-column align="center" label="订单状态" width="120">
          <template #default="{ row }">
            <el-tag v-if="row.state === '已发货'" class="status-tag" effect="light" round type="success">
              已发货
            </el-tag>
            <el-tag v-else-if="!row.state || row.state === '待发货'" class="status-tag" effect="light" round
                    type="warning">
              待发货
            </el-tag>
            <el-tag v-else class="status-tag" effect="light" round type="info">
              {{ row.state }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    >
      ...
      <el-form :model="sendForm" class="dialog-form" label-position="top" size="large">
          ...
          <el-row :gutter="15">
            <el-col :span="10">
              <el-form-item label="快递公司">
                <el-select v-model="sendForm.courierCompany" class="custom-select" placeholder="请选择">
            ...
            <el-col :span="14">
              <el-form-item label="快递单号">
                <el-input v-model="sendForm.trackingNumber" class="custom-input" placeholder="输入快递单号"/>
              </el-form-item>
            </el-col>
          </el-row>
        </div>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button class="btn-cancel" @click="sendDialogVisible = false">取 消</el-button>
          <el-button :loading="sending" class="btn-save" type="primary" @click="confirmSend">确认发货</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
import {Delete, Refresh, Shop, SuccessFilled, Van} from '@element-plus/icons-vue'
...
.custom-radio-group :deep(.el-radio-button__original-radio:checked + .el-radio-button__inner) {
  background: #55ab62;
  color: white;
  box-shadow: 0 4px 10px rgba(85, 171, 98, 0.3) !important;
}
